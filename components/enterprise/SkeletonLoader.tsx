  ...
  return (
    <div className={cn("space-y-4 animate-pulse", className)}>
      {Array.from({ length: count }).map((_, i) => {
        ...
        if (type === 'profile') {
          return (
            <div key={i} className="h-48 rounded-3xl border border-white/10 bg-slate-900/40 p-6 flex items-center gap-6">
              <div className="h-24 w-24 rounded-3xl bg-white/10 shrink-0" />
