  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="space-y-4">
        {loading ? (
          <Card><CardContent className="p-8 text-center text-slate-400">Loading requests...</CardContent></Card>
        ) : requests.length === 0 ? (
          <Card><CardContent className="p-8 text-center text-slate-400">No avatar requests found for this filter.</CardContent></Card>
        ) : (
          requests.map((req) => (
            <Card key={req._id} className="border border-slate-800 bg-slate-900/50">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-center">
                      ...
                      <div
                        className="relative w-20 h-20 rounded-full overflow-hidden border-2 border-slate-700 bg-slate-800 cursor-pointer hover:scale-105 hover:border-slate-500 transition-all shadow-md group"
                        title="Click to view & zoom"
                        onClick={() => req.currentAvatar && setPreviewData({
                          url: req.currentAvatar,
                          title: `Current Avatar - ${req.hostUserObjId?.name || `Host #${req.hostId}`}`
                        })}
                      >
                        {req.currentAvatar ? (
                          <Image src={req.currentAvatar} alt="Current" fill className="object-cover group-hover:opacity-90 transition-opacity" unoptimized />
                        ) : (
                    ...
                    <div className="text-center">
                      ...
                      <div
                        className="relative w-20 h-20 rounded-full overflow-hidden border-2 border-purple-500 bg-slate-800 shadow-lg shadow-purple-500/20 cursor-pointer hover:scale-105 hover:border-purple-400 transition-all group"
                        title="Click to view & zoom"
                        onClick={() => setPreviewData({
                          url: req.requestedAvatar,
                          title: `Requested Avatar - ${req.hostUserObjId?.name || `Host #${req.hostId}`}`
                        })}
                      >
                        <Image src={req.requestedAvatar} alt="Requested" fill className="object-cover group-hover:opacity-90 transition-opacity" unoptimized />
                      </div>
                    </div>
                  </div>
                  ...
                  {req.status === 'pending' && (
                    <div className="flex flex-col gap-2 w-full md:w-auto">
                      {rejectingId === req._id ? (
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" className="bg-emerald-600 hover:bg-emerald-500 text-white" onClick={() => void handleApprove(req._id)}>
