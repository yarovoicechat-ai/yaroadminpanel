...
import {
    ...
    ChevronRight, Check, AlertCircle, HelpCircle, User, AlertTriangle, Lock, Key, Crown, File
} from 'lucide-react';
...
export interface TimelineEvent {
    ...
    timestamp: string;
    actor: string;
    type: 'submission' | 'owner_review' | 'interview' | 'approval' | 'rejection';
}
...
export default function OperatorRequestsPage() {
    ...
    const [requests, setRequests] = useState<OperatorRequestData[]>([]);
    ...
    // Sorting & Pagination
    const [sortColumn, setSortColumn] = useState<keyof OperatorRequestData>('srNo');
    const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);

    // Modals & Drawers
    const [selectedReq, setSelectedReq] = useState<OperatorRequestData | null>(null);
    ...
    const [confirmModal, setConfirmModal] = useState<{
        isOpen: boolean;
        type: 'accept' | 'reject';
        request: OperatorRequestData | null;
    }>({
        isOpen: false,
        type: 'accept',
        request: null
    ...
    const fetchRequests = useCallback(async () => {
        setLoading(true);
        try {
            const res = await apiClient.get('/api/ems/requests', { requestType: 'Operator Request' });
            ...
            if (res && res.success !== false) {
                const apiData: OperatorRequestData[] = items.map((item: any, idx: number) => {
                    const d = item.data || {};
                    ...
                    return {
                        ...
                        timeline: item.timeline || [
                            { id: '1', title: 'Operator Application Received', description: 'Application filed successfully', timestamp: item.createdAt || new Date().toISOString(), actor: 'System', type: 'submission' }
                        ]
                    };
                });
                setRequests(apiData);
    ...
    const handleRefresh = async () => {
        ...
        toast.success('Operator requests updated with latest data');
    };
    ...
    const exportToCSV = () => {
        ...
        const headers = [
            ...
            'Registration Date', 'Owner Review Status', 'Password', 'Status'
        ];
    ...
    // Sort Toggle Handler
    const handleSort = (col: keyof OperatorRequestData) => {
        if (sortColumn === col) {
    ...
    const handleOpenConfirm = (req: OperatorRequestData, type: 'accept' | 'reject') => {
        if (currentUser?.role === 'operator' && type === 'accept') {
            toast.error('Operators cannot approve or create another Operator account.');
            return;
        }
        setConfirmModal({
            isOpen: true,
    ...
    const submitAction = async () => {
        ...
        try {
            ...
            const payload = isAccept
                ? { comments: actionRemarks || 'Approved by Admin', operatorCode: assignedCode }
            ...
            if (response.success) {
                ...
                if (!isAccept || !response.data?.generatedCredentials) {
                    toast.success(isAccept ? `✅ Operator request for ${req.name} APPROVED!` : `❌ Request for ${req.name} REJECTED.`);
                }
    ...
    return (
        <div className="min-h-screen bg-slate-50/70 dark:bg-slate-950 text-slate-900 dark:text-slate-100 p-4 sm:p-6 lg:p-8 space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                                Review, verify documents, and approve Operator registration requests before activation.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {/* Main Data Table Card (Exact 20 Columns) */}
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={20} className="p-12 text-center">
                                        <div className="max-w-md mx-auto space-y-4">
                                            ...
                                            <div>
                                                <h3 className="text-base font-bold text-slate-800 dark:text-white">No Operator Requests Found</h3>
                                ...
                                paginatedRequests.map((req, idx) => (
                                    ...
                                    >
                                        ...
                                        {/* 18. Password Column (Masked by default with Show/Hide toggle & Copy button) */}
                                        <td className="p-3.5 text-center whitespace-nowrap">
                                        ...
                                        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-850 p-6 rounded-xl border border-blue-200/80 dark:border-slate-700 space-y-4">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2.5">
                                                    ...
                                                    <div>
                                                        <h4 className="text-sm font-bold text-slate-900 dark:text-white">Operator Login Credentials</h4>
                                                        <p className="text-xs text-slate-500 dark:text-slate-400">Secure login credentials assigned to this Operator</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                                                ...
                                                <div className="bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                                                    <p className="text-slate-400 mb-1">Operator Code</p>
                                                    <p className="font-mono font-bold text-blue-600 dark:text-blue-400">{selectedReq.operatorCode || 'N/A'}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                {detailTab === 'review' && (
                                    <div className="space-y-4 text-xs">
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-800/40 p-5 rounded-xl border border-slate-200/80 dark:border-slate-800 space-y-3">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    <div className="p-2 rounded-lg bg-pink-100 dark:bg-pink-900/50 text-pink-600">
                                                        <Crown className="w-4 h-4" />
                                                    </div>
                                                    <div>
                                                        <h4 className="font-bold text-slate-900 dark:text-white">Owner Review</h4>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-base font-bold text-slate-900 dark:text-white">
                                        {confirmModal.type === 'accept' ? 'Approve Operator Role' : 'Reject Application'}
                                    </h3>
                            ...
                            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-md ${
                                        confirmModal.type === 'accept'
                                ...
                                >
                                    ...
                                    <span>{confirmModal.type === 'accept' ? 'Approve & Activate' : 'Confirm Reject'}</span>
                                </button>
                            </div>
                        </motion.div>
                    </div>
