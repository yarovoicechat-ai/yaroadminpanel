...
export default function VerificationReportsPage() {
  ...
  const download = async (type: 'face' | 'kyc') => {
    try {
      const base = ['localhost', '127.0.0.1'].includes(window.location.hostname) ? (process.env.NEXT_PUBLIC_LOCAL_API_BASE_URL || 'http://localhost:3001') : (process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.mithichat.live');
